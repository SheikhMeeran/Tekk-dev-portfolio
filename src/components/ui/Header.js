...
//   // 🔥 Final dynamic tab selection logic
//   useEffect(() => {
...
//       setValue(6);
//     } else if (path === "/about" || path === "/revolution") {
//       setValue(0);
//     }
...
const Header = () => {
  ...
  const routes = useMemo(
    () => [
      ...
      { label: "Cyber Security", link: "/software", activeTabIndex: 2 },
      { label: "Software Development", link: "/websites", activeTabIndex: 3 },
      { label: "Partner", link: "/partner", activeTabIndex: 4 },
      { label: "Contact Us", link: "/contact", activeTabIndex: 5 },
    ],
    [handleHomeHover]
  );
  ...
  const DrawerMenu = (
    <>
      ...
      <Box sx={{ ml: "auto", display: { xs: "block", md: "none" } }}>
        <IconButton onClick={() => setOpenDrawer(!openDrawer)} aria-label="menu">
          <MenuIcon />
        </IconButton>
      </Box>
    </>
